/*
Este código é um plugin jQuery que adiciona uma funcionalidade de avaliação de estrelas a elementos HTML.
Ele começa definindo um objeto "r" que contém os valores padrão para as opções do plugin, como a quantidade 
de estrelas, as classes CSS para estrelas vazias, meias e cheias, a cor das estrelas e se o plugin é 
somente para leitura.
Em seguida, o plugin é estendido para a função principal "rating", que recebe um objeto de opções como 
parâmetro.
Dentro desta função, o código itera sobre cada elemento HTML selecionado pelo jQuery e adiciona a 
funcionalidade de avaliação de estrelas a cada um deles.
O plugin adiciona atributos ao elemento HTML e também adiciona event listeners para os eventos 
"mousemove" e "click", para permitir a avaliação pelo usuário.
O código também define uma função interna "getStars", que seleciona todas as estrelas do 
elemento HTML.
O plugin permite que as opções sejam personalizadas pelo usuário, passando um objeto de opções 
como parâmetro na chamada da função "rating". Se nenhum objeto de opções é passado, são utilizados 
os valores padrão definidos no objeto "r".
*/
! function(t) {
    var e = {};
